...
#4
O3.2018.processed<-subset(O3.2018,
                          select=c(Date, DAILY_AQI_VALUE, Site.Name, 
                                   AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE,
                                   SITE_LONGITUDE))
O3.2019.processed<-subset(O3.2019,
                          select=c(Date, DAILY_AQI_VALUE, Site.Name, 
                                   AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE, 
                                   SITE_LONGITUDE))
PM25.2018.processed<-subset(PM25.2018,
                            select=c(Date, DAILY_AQI_VALUE, Site.Name,
                                     AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE, 
                                     SITE_LONGITUDE))
PM25.2019.processed<-subset(PM25.2019,
                            select=c(Date, DAILY_AQI_VALUE, Site.Name, 
                                     AQS_PARAMETER_DESC, COUNTY, SITE_LATITUDE, 
                                     SITE_LONGITUDE))

#5
PM25.2018.processed$AQS_PARAMETER_DESC[PM25.2018.processed$AQS_PARAMETER_DESC
                                       =="Acceptable PM2.5 AQI & Speciation Mass"]<-"PM2.5"
PM25.2018.processed$AQS_PARAMETER_DESC[PM25.2018.processed$AQS_PARAMETER_DESC
                                       =="PM2.5 - Local Conditions"]<-"PM2.5"
PM25.2019.processed$AQS_PARAMETER_DESC[PM25.2019.processed$AQS_PARAMETER_DESC
                                       =="Acceptable PM2.5 AQI & Speciation Mass"]<-"PM2.5"
PM25.2019.processed$AQS_PARAMETER_DESC[PM25.2019.processed$AQS_PARAMETER_DESC
                                       =="PM2.5 - Local Conditions"]<-"PM2.5"

#6
write.csv(O3.2018.processed, row.names = FALSE, 
          file = "./Data/Processed/EPAair_O3_NC2018_processed.csv")
write.csv(O3.2019.processed, row.names = FALSE, 
...
#9
EPAair.summaries.spread<-pivot_wider(EPAair.summaries,names_from 
                                     = "AQS_PARAMETER_DESC", 
                                     values_from = "meanAQI")
#10
...
```{r}
#12(a,b)
EPAair.summaries2<-EPAair.summaries.spread%>%
  group_by(Site.Name,Month,Year)%>%
  summarise(meanAQI.ozone=mean(Ozone),meanAQI.pm25=mean(PM2.5))%>%
  filter(!is.na(meanAQI.ozone))%>%
  filter(!is.na(meanAQI.pm25))
