...
NTL_LTERplot<-ggplot(NTL_LTER,aes(x=po4,y=tp_ug, color=lakename))+
  geom_point()+
  geom_smooth(method = "lm",se=FALSE,color="black")+xlim(0,50)+labs(x="total phosphorus",y="phosphate")
print(NTL_LTERplot)
```
...
```{r}
#5
temp.boxplot<-ggplot(NTL_LTER,aes(x=month,y=temperature_C,color=lakename))+
  geom_boxplot()
print(temp.boxplot)
TP.boxplot<-ggplot(NTL_LTER,aes(x=month,y=tp_ug,color=lakename))+
  geom_boxplot()
print(TP.boxplot)
TN.boxplot<-ggplot(NTL_LTER,aes(x=month,y=tn_ug,color=lakename))+
  geom_boxplot()
print(TN.boxplot)
...
> Answer: I found that temperature, TP, and TN vary as seasons and lake vary. Temperature, TP and TN only have values in Spring for lake Paul and other values in Fall for lake Peter. 
