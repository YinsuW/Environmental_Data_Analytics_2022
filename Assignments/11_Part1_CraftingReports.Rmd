...
- ***embedded plots and tables that are easily updated***
...
----------------------------|---------------------------
NWIS_SiteInfo_NE_RAW.csv    | NWS site information
...
```{r setup, message=FALSE, echo=FALSE,message=FALSE,warning=FALSE}
...
```{r echo=TRUE}
NTL_LTER_processed<-subset(NTL_LTER,select = -c(lakeid,depth_id,comments))
NTL_LTER_processed<-filter(NTL_LTER_processed,depth == 0)%>%drop_na()
```
...
```{r include=FALSE}
NTL_LTER_summary_1<-NTL_LTER_processed%>%
  group_by(lakename)%>%
  summarise(N.mean=mean(tn_ug),
            N.min=min(tn_ug),
            N.max=max(tn_ug),
            N.sd=sd(tn_ug))

NTL_LTER_summary_2<-NTL_LTER_processed%>%
  group_by(lakename)%>%
  summarise(P.mean=mean(tp_ug),
            P.min=min(tp_ug),
            P.max=max(tp_ug),
            P.sd=sd(tp_ug))
```
...
NTL_LTER_table_1<-kable(NTL_LTER_summary_1, format="simple",booktabs=TRUE, caption="Table 2: Surface Samples: Total Nitrogen")
print(NTL_LTER_table_1)
```


```{r echo=FALSE}
NTL_LTER_table_2<-kable(NTL_LTER_summary_2, format="simple",booktabs=TRUE,caption="Table 3: Surface Samples: Total Phosphorous")
print(NTL_LTER_table_2)
```
...
```{r echo=FALSE, fig.align="left", fig.cap="Figure 1: Total Nitrogen", message=FALSE, warning=FALSE}
plot_N<-ggplot(NTL_LTER_processed,aes(y=tn_ug,x=sampledate,color=lakename))+
  geom_point(alpha=0.3)+
  geom_smooth(se=F)+
  labs(y="Total N",x="Date",color="Lake", title = "Total N over time")
print(plot_N)
```

```{r echo=FALSE, fig.align= "left", fig.cap="Figure 2: Total Phosphorous", message=FALSE, warning=FALSE}
plot_P<-ggplot(NTL_LTER_processed,aes(y=tp_ug,x=sampledate,color=lakename))+
  geom_point(alpha=0.3)+
  geom_smooth(se=F)+
  labs(y="Total P",x="Date",color="Lake", title = "Total P over time")
print(plot_P)
```
...
>From the results we can see that for most lakes in the dataset, total nitrogen and total phosphorous increase first and then decrease over time. 

***
>I would like to explore whether total nitrogen and total phosphorous are the same across different depths of lake using ANOVA analysis. 
